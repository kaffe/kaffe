/* DecoderUTF32LE.java -- Decoder for the UTF-32 Little Endian character set.
 ...
 * Big Endian (UTF-32LE) encoding scheme.
 * 
 * @version 0.0
 * @author Quentin Anciaux (quentin.anciaux@advalvas.be)
 */
public class DecoderUTF32LE extends Decoder
{
  ...
  public DecoderUTF32LE(InputStream in)
  {
    super(in, "UTF-32LE");
  }
	...
	if (i + 3 < buf.length)
	  {
		...
		{ buf[i + 3], buf[i + 2], buf[i + 1], buf[i] }).longValue();
  ...
  /**
   * Transform the specified UTF32LE encoded buffer to Unicode characters
   */
  ...
  /**
   * Reads chars from a UTF32LE encoded byte stream
   */
  public int read(char[] cbuf, int offset, int len) throws IOException
  {
	...
	long dword = new BigInteger(new byte[]
	    { buffer[3], buffer[2], buffer[1], buffer[0] }).longValue();
