/* EncoderUTF32BE.java -- Encoder for the UTF-32 Big Endian character encoding.
 ...
 */
public class EncoderUTF32BE extends Encoder
{

  public EncoderUTF32BE(OutputStream out)
  {
    super(out, "UTF32BE");
  }
			       ...
			       byte[] bbuf, int bbuf_offset)
  {
	...
	if (i+1 < buf_offset + len && ((int) buf[i]) >= 0xD800
	  ...
	  {
	    byte [] newArray;
	    newArray = new BigInteger("" + ((int) buf[i])).toByteArray();
	    if (newArray.length < 2)
	      {
		bbuf[bbuf_offset++] = 0;
		bbuf[bbuf_offset++] = 0;
