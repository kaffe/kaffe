          ...
          if (file.contains(user))
            {
              ...
              BigInteger v, N, g;
              v = new BigInteger(1, gnu.crypto.util.Util.fromBase64(ent[0]));
              N = new BigInteger(1, gnu.crypto.util.Util.fromBase64(cnf[0]));
              g = new BigInteger(1, gnu.crypto.util.Util.fromBase64(cnf[1]));
      ...
      try
        {
          if (file.contains(user))
            {
              return gnu.crypto.util.Util.fromBase64(file.lookup(user, "SHA")[1]);
            }
        }
      catch (IOException ioe) { }
      return null;
          ...
          if (file.contains(user))
            {
              return new BigInteger(1,
                gnu.crypto.util.Util.fromBase64(file.lookup(user, "SHA")[0]));
            }
        }
      catch (IOException ioe) { }
      return null;
