...
struct call_graph *createCallGraph(int count)
{
	...
	if( (retval = (struct call_graph *)
	    ...
	    (arcs = (struct call_arc *)
	     malloc(sizeof(struct call_arc) * count)) )
	{
		int lpc;
...
int writeCallGraph(struct call_graph *cg, struct gmon_file *gf)
{
	...
	for( lpc = 0; (lpc < cg->cg_count) && retval; lpc++ )
	{
...
#if defined(KAFFE_CPROFILER)
	/*
	 * Here we write out any arcs that were generated by the C compiler's
	 * mcount.
	 */
		...
		if( gp->froms[lpc] )
		{
			     ...
			     to_index = gp->tos[to_index].link )
			{
				retval = writeGmonRecord(
					...
					GRA_Type, GMON_TAG_CG_ARC,
					GRA_FromPC, frompc,
					GRA_SelfPC, gp->tos[to_index].selfpc,
					GRA_Count, gp->tos[to_index].count,
					GRA_DONE);
			}
		}
	}
