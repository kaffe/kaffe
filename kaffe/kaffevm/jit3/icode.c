...
load_addr_int(SlotInfo* dst, void* addr)
{
	...
	slot_slot_const(dst, 0, (uintp)addr, HAVE_load_addr_int, Tload);
#else
	SlotInfo* tmp;
	slot_alloctmp(tmp);
...
load_addr_ref(SlotInfo* dst, void* addr)
{
	...
	slot_slot_const(dst, 0, (uintp)addr, HAVE_load_addr_ref, Tload);
#else
	SlotInfo* tmp;
	slot_alloctmp(tmp);
	move_ref_const(tmp, addr);
...
load_addr_float(SlotInfo* dst, void* addr)
{
	...
	slot_slot_const(dst, 0, (uintp)addr, HAVE_load_addr_float, Tload);
#else
	SlotInfo* tmp;
	slot_alloctmp(tmp);
	move_ref_const(tmp, addr);
...
load_addr_double(SlotInfo* dst, void* addr)
{
	...
	slot_slot_const(dst, 0, (uintp)addr, HAVE_load_addr_double, Tload);
#else
	SlotInfo* tmp;
	slot_alloctmp(tmp);
	move_ref_const(tmp, addr);
...
load_offset_scaled_double(SlotInfo* dst, SlotInfo* src, SlotInfo* idx, int offset)
{
	...
	slot_slot_slot_const_const(dst, src, idx, offset, 0, HAVE_load_offset_scaled_double, Tload);
#else
	{
		SlotInfo* tmp;
		slot_alloctmp(tmp);
...
store_addr_int(void* addr, SlotInfo* src)
{
	...
	slot_slot_const(0, src, (uintp)addr, HAVE_store_addr_int, Tstore);
#else
	SlotInfo* tmp;
	slot_alloctmp(tmp);
	move_ref_const(tmp, addr);
...
store_addr_ref(void* addr, SlotInfo* src)
{
	...
	slot_slot_const(0, src, (uintp)addr, HAVE_store_addr_ref, Tstore);
#else
	SlotInfo* tmp;
	slot_alloctmp(tmp);
	move_ref_const(tmp, addr);
...
store_addr_double(void* addr, SlotInfo* src)
{
	...
	slot_slot_const(0, src, (uintp)addr, HAVE_store_addr_double, Tstore);
#else
	SlotInfo* tmp;
	slot_alloctmp(tmp);
	move_ref_const(tmp, addr);
...
store_offset_scaled_double(SlotInfo* dst, SlotInfo* idx, int offset, SlotInfo* src)
{
	...
	slot_slot_slot_const_const(dst, idx, src, offset, 0, HAVE_store_offset_scaled_double, Tstore);
#else
	{
		SlotInfo* tmp;
		slot_alloctmp(tmp);
...
cbranch_int_const(SlotInfo* s1, jint val, label* dst, int type)
{
	...
	if (HAVE_cbranch_int_const_rangecheck(val)) {
		slot_slot_const_const_const(0, s1, val, (jword)dst, type, HAVE_cbranch_int_const, Tcomplex);
	}
	else {
		SlotInfo* tmp;
		slot_alloctmp(tmp);
...
check_null(int x, SlotInfo* obj, int y)
{
	...
	{
		end_sub_block();
		cbranch_ref_const_ne(obj, 0, reference_label(x, y));
...
softcall_fakecall(label* from, label* to, void* func)
{
	 ...
	 * got here from.
	 */
	slot_const_const(0, (jword)from, (jword)l, HAVE_fakecall, Tnull);
#endif
}
